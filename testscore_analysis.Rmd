...
####some queries below using dcast need some dates changed
...
state_read <- statewide %>%  filter(data_year>='12-13', subject=='R',report_description=='All Students') %>% group_by(data_year, subject) %>% 
...
state_math <- statewide %>%  filter(data_year>='12-13',subject=='M',report_description=='All Students') %>% group_by(data_year, subject) %>% 
  summarise(tottested=sum(count_tested), totlevel1=sum(count_level1), totlevel2=sum(count_level2),  totlevel3=sum(count_level3), totlevel4=sum(count_level4)) %>% mutate(countproficient=totlevel3+totlevel4, pctproficient=countproficient/tottested)
...
state_race <-  statewide %>% filter(data_year>='13-14', report_category=='Race/Ethnicity' | report_category=='All Categories', subject=='M') %>%  group_by(subject, data_year, report_description) %>% summarise(tot=sum(count_tested), prof = sum(count_level3)+sum(count_level4)) %>% mutate(pct= (prof/tot)) %>% select(data_year, report_description, pct) %>% filter(report_description!='P-Native Hawaiian/Pacific Islander')
...
## Race groups- reading
```{r}
state_race_read <-  statewide %>% filter(data_year>='13-14',report_category=='Race/Ethnicity' | report_category=='All Categories', subject=='R') %>%  group_by(subject, data_year, report_description) %>% summarise(tot=sum(count_tested), prof = sum(count_level3)+sum(count_level4)) %>% mutate(pct= (prof/tot)) %>% select(data_year, report_description, pct) %>% filter(report_description!='P-Native Hawaiian/Pacific Islander')
...
```{r}
testscores %>% filter(data_year==thisyear, poverty_category=='High') %>% group_by(categoryname) %>% summarise(count=n()) %>% mutate(pct=round(count/sum(count)*100,1))%>%  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```

##high poverty schools in math
```{r}
testscores %>% filter(poverty_category=="High", subject=='M') %>% tabyl(data_year, categoryname) %>% 
  #adorn_totals("col") %>%
...
```{r}
testscores %>% filter(poverty_category=="High", subject=='R') %>% tabyl(data_year, categoryname) %>% 
  #adorn_totals("col") %>%
  adorn_percentages("row") %>%
  adorn_pct_formatting(rounding = "half up", digits = 0) %>%
   adorn_ns() %>% 
...
fallingshortmetro <- testscores %>% filter(data_year==thisyear, poverty_category=='High', categoryname=='Falling short', metro7_strib=='YES') %>% select(subject, districtname, schoolname,  pctprof, predicted, residual, pctminority, pct_poverty, districttype) %>% arrange((schoolname))

betterthanexpectedmetro <- testscores %>% filter(data_year==thisyear, poverty_category=='High', categoryname=='Better than expected', metro7_strib=='YES') %>%  select(subject, districtname, schoolname,  pctprof, predicted,  pct_poverty, districttype)%>% arrange((schoolname)) %>% mutate(pct_prof= round(pctprof*100,1),                                                                                                  predict = round(predicted*100,1)) %>% select(-pctprof, -predicted) 
...
```{r}
testscores %>% filter(subject=='M', metro7_strib=='YES', districttype!='07', data_year=='17-18') %>% tabyl(districtname, categoryname) %>%
  adorn_percentages("row") %>%
  adorn_pct_formatting(rounding = "half up", digits = 0) %>%
   adorn_ns() %>% 
   adorn_title("top")%>%
...
math_scatterplot <- ggplot(testscores_math, aes(x = pct_poverty, y = pctprof)) +
...
scatter_math <-  testscores_math %>% select(schoolid, categoryname, pct_poverty, pctprof) %>% mutate(pctpov = round(pct_poverty*100,2),
                                                                                       prof = round(pctprof*100,2)) %>% select(-pct_poverty, -pctprof)
...
#here's the scatterplot
#notice that we didn't need to make a separate dataframe; we're pulling from the main one
read_scatterplot <- ggplot(testscores_read, aes(x = pct_poverty, y = pctprof)) +
  theme_minimal(base_size = 14, base_family = "Arial") + 
  geom_point(size = 1, alpha = 0.5, aes(color=categoryname)) + 
  scale_y_continuous(labels = percent, limits=c(0,1))+
  scale_x_continuous(labels = percent, limits=c(0,1))+
...
scatter_read <-  testscores_read %>% select(schoolid, categoryname, pct_poverty, pctprof) %>% mutate(pctpov = round(pct_poverty*100,2),
                                                                                       prof = round(pctprof*100,2)) %>% select(-pct_poverty, -pctprof)
...
```{r}
testscores %>% filter(data_year==thisyear, poverty_category=='High', categoryname=='Better than expected') %>% group_by(subject) %>% summarise(prof=sum(numproficient), tot=sum(cnt_tested)) %>% mutate(pct=round((prof/tot)*100,1)) %>% select(subject, pct)%>%  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
...
```{r}
testscores %>% filter(data_year==thisyear, poverty_category=='High') %>% group_by(subject) %>% summarise(prof=sum(numproficient), tot=sum(cnt_tested)) %>% mutate(pct=round((prof/tot)*100,1)) %>% select(subject, pct)%>%  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```

#Opt outs
##Total opt outs, math & reading combined
```{r}
optouts %>% filter(report_description=='All Students') %>% group_by(data_year) %>% summarise(tot_optout=sum(optout), tot_students=sum(grade_enrollment)) %>% mutate(per10k = round((tot_optout/tot_students)*10000,1))%>%  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```
...
##Math opt outs
opt out rates are based on all schools, even those whose test scores were suppressed
```{r}


ggplot(math_optouts, aes(x=reorder(variable, per10k), y=per10k)) + 
  geom_bar(stat="identity", color="black", fill="#3e0045")+
  scale_y_continuous(limits=c(0,400))+
  coord_flip()+
  theme_hc() +
  labs(title = "Math opt out rates by group", 
       subtitle = thisyear,
       caption = "Graphic by MaryJo Webster",
       x="Grade",
       y="Rate per 10,000")
...
ggplot(read_optouts, aes(x=reorder(variable, per10k), y=per10k)) + 
  geom_bar(stat="identity", color="black", fill="#3e0045")+
  scale_y_continuous(limits=c(0,400))+
  coord_flip()+
  theme_hc() +
  labs(title = "Read opt-out rates by group", 
       subtitle = thisyear,
       caption = "Graphic by MaryJo Webster",
       x="Grade",
       y="Rate per 10,000")
...
##Statewide math optouts
```{r}
optouts %>% filter(report_order=='1', subject=='M') %>% group_by(data_year) %>% summarize(tot_optout = sum(optout), tot_students = sum(grade_enrollment)) %>% mutate(per10k = round((tot_optout/tot_students)*10000,1))%>%  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```

##statewide reading optouts
```{r}
optouts %>% filter(report_order=='1', subject=='R') %>% group_by(data_year) %>% summarize(tot_optout = sum(optout), tot_students = sum(grade_enrollment)) %>% mutate(per10k = round((tot_optout/tot_students)*10000,1))%>%  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```

##Metro opt outs in Math
```{r}
optouts %>% filter(report_order=='1', subject=='M', metro7_strib=='YES') %>% group_by(data_year) %>% summarize(tot_optout = sum(optout), tot_students = sum(grade_enrollment)) %>% mutate(per10k = round((tot_optout/tot_students)*10000,1))%>%  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```
##Metro opt outs in Reading
```{r}
optouts %>% filter(report_order=='1', subject=='R', metro7_strib=='YES') %>% group_by(data_year) %>% summarize(tot_optout = sum(optout), tot_students = sum(grade_enrollment)) %>% mutate(per10k = round((tot_optout/tot_students)*10000,1))%>%  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```

##Grade 11 math opt outs
```{r}
optouts %>% filter(report_order=='1', subject=='M', metro7_strib=='YES', grade=='11') %>% group_by(data_year) %>% summarize(tot_optout = sum(optout), tot_students = sum(grade_enrollment)) %>% mutate(per10k = round((tot_optout/tot_students)*10000,1))%>%  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```
...
dist_math_wide <-  dist_math_wide %>% mutate(diff = x17_18-x16_17)
...
dist_read_wide <-  dist_read_wide %>% mutate(diff = x17_18-x16_17)
...
##top level numbers
```{r}
districts_mca %>% filter(districtid=='0709-01-000') %>% select(datayear, subject, report_description, pctprof) %>% mutate(prof = round(pctprof*100,1)) %>% select(-pctprof)%>% arrange(subject, datayear) %>% 
  knitr::kable("html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, position="left")
```
...
```{r}
testscores %>% filter(districtnumber=='0709', data_year==thisyear) %>% select(data_year, schoolname, subject, poverty_category,categoryname, pctprof ) %>% 
...
duluth_math <-  dcast(duluth_math, schoolname ~ data_year) %>% clean_names() %>% mutate(change=round((x17_18-x16_17),1))
...
duluth_read <-  dcast(duluth_read, schoolname ~ data_year) %>% clean_names() %>% mutate(change=round((x17_18-x16_17),1))
...
mps_math <-  dcast(mps_math, schoolname ~ data_year) %>% clean_names() %>% mutate(change=round((x17_18-x16_17),1))
...
mps_read <-  dcast(mps_read, schoolname ~ data_year) %>% clean_names() %>% mutate(change=round((x17_18-x16_17),1))
...
sp_math <-  dcast(sp_math, schoolname ~ data_year) %>% clean_names() %>% mutate(change=round((x17_18-x16_17),1))
...
sp_r <-  dcast(sp_r, schoolname ~ data_year) %>% clean_names() %>% mutate(change=round((x17_18-x16_17),1))
...
ah_math <-  dcast(ah_math, schoolname ~ data_year) %>% clean_names() %>% mutate(change=round((x17_18-x16_17),1))
...
ah_read <-  dcast(ah_read, schoolname ~ data_year) %>% clean_names() %>% mutate(change=round((x17_18-x16_17),1))
...
put_object(file = "testscore_analysis.html", object = "projects/schools/testscore_analysis.html", bucket = "strib-data-internal")

put_object(file = "testscore_analysis.html", object = "projects/schools/testscore_analysis.html", bucket = "strib-data-internal", acl=c("public-read"))
