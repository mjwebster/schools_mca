...
# import that to the mca2019_present table in mySQL server
...
#import data from mySQL  (be sure to add the newest data first)
```{r}
...
mca_mtas <-  left_join(mca_mtas, master_list %>% select(-district_type, -school_classification), by=c("idnumber"="idnumber"))
...
public_enrollment <- read_excel(public_enroll_file, sheet="School", skip=1) %>%
  clean_names() %>% filter(grade=='All Grades') %>% mutate(schoolid=paste(district_number, district_type, school_number, sep="-"))
...
public_poverty <-  public_enrollment %>% filter(grade=='All Grades')  %>% 
  select(schoolid ,total_enrollment, poverty = total_students_eligible_for_free_or_reduced_priced_meals_count, pct_poverty=total_students_eligible_for_free_or_reduced_priced_meals_percent,
         pct_white = total_white_percent) %>% 
  mutate(poverty_calc = case_when(pct_poverty!='NA' ~ poverty/total_enrollment),
         povertylevel = case_when(pct_poverty=='>=90%' ~ 'high',
                                  poverty_calc >=.6 ~ 'high',
                                  poverty_calc<.6 & poverty_calc>.3 ~ 'medium',
                                  poverty_calc<=.3  ~ 'low',
                                  is.na(pct_poverty) & pct_white< 50 ~ 'likely high',  #these are schools with low share of white
                                  is.na(pct_poverty) & pct_white>=50 ~ 'likely low',  #these are schools 50% or more white
                                  TRUE ~ 'unk'))




#join poverty info to mca_mtas
mca_mtas <-  left_join(mca_mtas, public_poverty, by=c("idnumber"="schoolid"))


#public_poverty %>% filter(is.na(poverty_calc)) %>% arrange(desc(total_enrollment))

  mca_mtas$poverty_calc[mca_mtas$pct_poverty=='>=90%'] <- .90


#clean leading/trailing spaces on key fields
mca_mtas <-  mca_mtas %>% mutate(grade= str_trim(grade), subject=str_trim(subject), student_group=str_trim(student_group))
...
mca_mtas <- mca_mtas%>% mutate(pct_pov_public = case_when(is.na(pct_poverty)~'Not public',
...
```{r}
#math results, all students, all grades collapsed, school level where 25 or more enrollment, not filtered

math <- mca_mtas %>% filter(subject=='MATH', grade=='00', level=='school', student_group=='All students', filter_all=='N', total_tested>=25, poverty_calc!='NA') %>% mutate(pct_proficient = (count_level_m+count_level_e)/total_tested) 


read <- mca_mtas %>% filter(subject=='Reading', grade=='00', level=='school', student_group=='All students', filter_all=='N', total_tested>=25, poverty_calc!='NA')%>% mutate(pct_proficient = (count_level_m+count_level_e)/total_tested)
...
math_excluded <-  mca_mtas %>% filter(subject=='MATH', grade=='00', level=='school', student_group=='All students', filter_all=='N',total_tested<25 | is.na(poverty_calc))%>%
  mutate(numproficient=count_level_m+count_level_e, pct_proficient= (count_level_m+count_level_e)/total_tested, 
         notes='Less than 25 students tested', predicted=NA_real_,  residual=NA_real_)


read_excluded <- mca_mtas %>% filter(subject=='Reading', grade=='00', level=='school', student_group=='All students', filter_all=='N',total_tested<25 | is.na(poverty_calc)) %>%
  mutate(numproficient=count_level_m+count_level_e, pct_proficient= (count_level_m+count_level_e)/total_tested, 
         notes='Less than 25 students tested', predicted=NA_real_,  residual=NA_real_)
...
#test to make sure all the schools are here; the second query should result in zero

combined <-  bind_rows(math, read, math_excluded, read_excluded, missing)
...
math <-  math %>% mutate(categoryname= case_when(categorynum==99~"Not enough students tested", categorynum==1~"Falling short", categorynum==2~"As expected", categorynum==3~"Better than expected", TRUE~"99"))
...
read <-  read %>% mutate(categoryname= case_when(categorynum==99~"Not enough students tested", categorynum==1~"Falling short", categorynum==2~"As expected", categorynum==3~"Better than expected", TRUE~"99"))
...
#exclude alternative learning centers; classification codes 41, 42 and 43
testscores <- bind_rows(read, math, read_excluded, math_excluded, missing) %>% filter(school_classification!='41', school_classification!='42', school_classification!='43')
...
testscores_public <- testscores %>% 
  select(uniqueID, schoolid=idnumber, districtnumber=district_number, districttype=district_type, school_number, districtname=standard_district_name, schoolname=standard_school_name, schoolclassification=school_classification, school_type,
         ...
         cntlev4=count_level_e, numproficient, pctprof=pct_proficient, total_enrollment, pct_pov_private=poverty_calc, pct_pov_public, predicted, residual, notes, categorynum, 
